...
TEAMSCENTRAL CENTRALCLUSTERING::Initialization(){
  ...
  centrals.teams_euclidian = new vector<POINTDISTANCE>[this->k]; // The euclidian Distance of all points to center i for k clusters
  ...
  while(lead < this->k){ // Create 2 to k centers
    ...
    for(int i = 0; i < this->how_many; i++){
      ...
      pd.point = i;
      centrals.teams_euclidian[lead].push_back(pd);
      ...
      if(CheckIfDifferent(leaders_central, i) == true){ // If i is not a leader in a cluster then find the average distance
        ...
        for(int j = 0; j < lead; j++)
          distance_all = distance_all + centrals.teams_euclidian[j][i].distance;
  ...
  for(int i = 0; i < this->k; i++){
    centrals.leaders.push_back(all_p[leaders_central[i]]); // Add centers of cluster to return class object
    sort(centrals.teams_euclidian[i].begin(), centrals.teams_euclidian[i].end(), PDCompareFunctionPoint());
  }

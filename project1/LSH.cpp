...
LSH::LSH(double **all_p1, int how_many1, int N1 = 1, int dimensions1 = 784, int k1 = 4, int r1 = 10000, int l1 = 5){
  ...
  this->id = new int*[this->l];
  for(int i = 0; i < this->l; i++){
    this->id[i] = new int[this->how_many];
    this->g[i] = new int[this->how_many];
  }
  ...
  for (int i = 0; i < this->l; i++){
    ...
    memcpy(this->g[i], this->g_temp, this->how_many*sizeof(int));
    memcpy(this->id[i], this->id_temp, this->how_many*sizeof(int));
  }
}
...
void LSH::HashTable(int w, int n, int k, double *q) {
  ...
  int M = UINT_MAX - 5;
  int table_size = this->how_many / 8;
  int r[k];
...
vector<POINTDISTANCE> LSH::NNearestNeighbors(double* q){
  ...
  for(int i = 0; i < this->l; i++){
    for(int j = 0; j < this->how_many; j++){
      if (this->g[i][j] == g_q && (this->id[i][j] == id_q)){
        if (count(number_of_index_equal_id.begin(), number_of_index_equal_id.end(), j) == 0)
          number_of_index_equal_id.push_back(j);
      } else if(this->g[i][j] == g_q){
        if (count(number_of_index_equal_g.begin(), number_of_index_equal_g.end(), j) == 0)
          number_of_index_equal_g.push_back(j);
      }
    }
  }
